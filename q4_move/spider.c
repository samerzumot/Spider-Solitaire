...
// is_full(cards) checks if a stack of cards is in the correct sequence 
// and returns a boolean value
bool is_full(struct stack cards) {

  int cardslen = stack_len(cards);
  for(int i = 1; i <= 13; ++i) { 
    if (cardslen == 0) {
      return false;
    } 
    --cardslen;
    if (!(i == stack_pop(&cards))) {
      return false;
    }
  }
  return true;
...
void onewin(struct spider *s) {
  assert(s);
  for(int i = 0; i <= 9; ++i){
    if (is_full(s->tableau_up[i]) == true) {
      ++s->comp_decks;
      for(int k = 0; k < 13; ++ k) {
        stack_pop((&(s->tableau_up[i])));
      }
    }
  }
}
...
void spider_test_find_seq(void) {
  ...
  found = spider_find_seq(&src, &temp, card);
  assert(!found);
  assert(stack_pop(&temp) == 15);
  assert(stack_pop(&temp) == 24);
  assert(stack_pop(&temp) == 33);
  assert(stack_pop(&temp) == 0);
  assert(stack_is_empty(&temp));
  assert(stack_is_empty(&src));
...
// face_up(s) opens up the cards when the end of the tableau_up stack
// has been reached
// effects: may mutate *s
// requires: s is a non-NULL pointer
void face_up(struct spider *s){
  assert(s);
  for(int i = 0; i < 10; ++i){
...
void spider_move(struct spider *s, 
  ...
  stack_init(&temporary);

  bool is_valid1 = ((spider_find_seq(&s->tableau_up[src],
                                     &temporary, card)) ||
                    (stack_is_empty(&(s->tableau_up[src]))));
  bool is_valid2 = ((stack_is_empty(&(s->tableau_up[dest]))) ||
                    (stack_top(&(s->tableau_up[dest])) ==
                     card+1));
  int lentemp = stack_len(temporary);
  if (is_valid1 == true && is_valid2 == true) {
    for(int i = 0; i < lentemp; i++) {
      stack_push(stack_pop(&temporary), &(s->tableau_up[dest]));
    }
  } else {
    printf("Invalid move.\n");
  }
